...
object Adaptor {
  ...
  // RxScala users must not import rx.Observable, because it hides the companion object.
  // Instead, they import rx.lang.scala.Adaptor._, to get a type and an object both named
  // `Observable` and happily coexisting.
  ...
  implicit class ScalaBlockingObservable[T](val wrapped: JBlockingObservable[T]) extends AnyVal {
...
class UnitTestSuite extends JUnitSuite {
    ...
    */
    ...
    @Test def testLast {
        val observable = Observable(1, 2, 3, 4).toBlockingObservable
    ...
    // TODO same IMPORTANT problem as above:
    // type mismatch; found : Int => Int required: rx.util.functions.Func1[Int,?]
    /*
